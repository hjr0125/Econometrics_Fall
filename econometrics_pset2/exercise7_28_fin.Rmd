...
In some cases we want to estimate m(x) at a particular point x. Notice that this is a linear function of $\beta$. Letting $r (\beta) = x'\beta$ and $\theta = r (\beta)$ we see that $\hat{m} (x) = \hat{θ} = x'\hat{\beta}$ and $R = x$ so $s(\hat{\theta}) = \sqrt{x'{\hat{\textbf{V}}}_{\hat{\beta}} x}$
